...
// Classe graphics qui g�re l'affichage du jeu
...
public class BirdGraphics extends JFrame implements ActionListener{
	...
	public Resultat R;
	int tps=0; // Temps d'attente avant affichage de la fen�tre r�sultat

	public BirdGraphics(BirdWorld p, int Niveau) { // Fen�tre principale du jeu, gameplay
		super("Angry Birds"); 
		pj = new Niveau(p); // cr�ation du niveau choisi
		board = p;
		add(pj); 
		setSize(1000,800);
		...
		ActionListener move = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				if (c1==0) { // Premiere it�ration, on fait apparaitre les objets
					afficherPlateau(p);	
				}
				if (c1==1 && c==0) {// Si l'utilisateur clique le bird suit sa souris
					if (MouseInfo.getPointerInfo().getLocation().x-63>=450) { // limiter le lancement sur la droite pour �viter que l'utilisateur puisse tirer l'oiseau n'importe o� directement dans les structures 
						board.initX=450;
					}
				...
				if (c>0) {// Si l'utilisateur relache la souris, l'oiseau est lanc�
					board.movePiece();
					afficherPlateau(p);	
					if (board.testend()&&mf==0) { // Affichage de la fen�tre resultat en cas de fin de niveau 
						tps+=1;
						if (tps>10) {
							mf=1;
	...
	// Actions lors du click sur les boutons "Quitter" (la croix) et "Restart"(symbole restart) en haut de la fen�tre
	public void actionPerformed (ActionEvent e){
		if (e.getSource()==B1) { // Bouton Quitter
			if (R!=null && R.isVisible()==true){
				R.dispose();
			}
			sgame.stop(); // Arr�t du jeu (world du moteur physique)
			t.stop(); // Arr�t du timer
			// Affichage en vert du bouton du niveau lorsqu'il est r�ussie
			...
			this.dispose();
			board.m.SG.setVisible(true); // R�affichage du menu start 

		}
		if (e.getSource()==B2) { // Bouton Restart
			// Arr�t du jeu en cours
			if (R!=null && R.isVisible()==true){
				R.dispose();
			}
			t.stop();
			this.dispose();
			sgame.stop(); 
			// Relance un nouveau jeu avec ses param�tres initiaux
	...
	public void afficherPlateau(BirdWorld p) { 
		if (worldGraphics==null) // Si le monde n'est pas encore affich�
	...
	class Niveau extends JPanel {
		...
		private void dessineMonde(Graphics g) {
			...
			// On adapte le fond d'�cran en fonction de la gravit� (gvt)
			...
			if (c<1) {
				if (board.Niveau == 1) {
					...
					g.drawString("Goal: Eliminate the 3 pigs", 580, 300);
				...
				if (board.Niveau == 2) {
					...
					g.drawString("Goal: Eliminate at least 2 pigs", 500, 200);
				}
				if (board.Niveau == 3) {
					...
					g.drawString("Goal: Eliminate the target", 450, 320);
				}
				if (board.Niveau == 4) {
					y=t.getImage(getClass().getResource("/resources/arrowright.png"));
					g.drawImage(y,380,350,60,46,null);
					g.drawString("Goal: Eliminate the target", 140, 280);
				}

				if (board.Niveau == 5) {
					y=t.getImage(getClass().getResource("/resources/arrowdown.png"));
					g.drawImage(y,380,520,70,70,null);
					g.drawString("Goal: Eliminate the two pigs", 140, 340);
				}
				if (board.Niveau == 6) {
					y=t.getImage(getClass().getResource("/resources/arrowdown.png"));
					g.drawImage(y,560,100,70,70,null);
					g.drawString("Goal: Eliminate all the pigs", 480, 70);

				}
			}
			// Trac� des traits repr�sentant l'�lastique lors du tire de l'oiseau
			if (c1==1 && c==0) {
			...
			// Draw le bird toujours droit (mais en meilleur r�solution)
			...
			// D�finit les couleurs en fonction du mat�riaux choisi en options
			...
			// Dessin des Briques (murs et sol) cr�s dans BirdWolrd pour chaque Niveau
			...
			if (Niveau == 1 || Niveau ==2 || Niveau ==6) {
				...
				// Dessin des Pigs pour chaque Niveau tant qu'ils ne sont pas d�truits
	...
	/* M�thode affichant, dessinant un rectangle correspondant parfaitement au body du moteur physique
	   En r�alit� , cette m�thode permet de dessiner tout type de polygone, et pas seulement des rectangles
	...
	public void drawRectangleObject(Body b, PolygonShape gb, Graphics g, Color color){
		...
		// R�cup�ration des coordonn�es de tous les sommets du body 
		for (int i=0; i<gb.m_count; i++) {
			xp[i]=(int)((gb.m_vertices[i].x+b.getPosition().x)*100);
			yp[i]=(int)((gb.m_vertices[i].y+b.getPosition().y)*100);
		}
		Polygon rect1 = new Polygon(xp, yp, gb.m_count); // Cr�ation du polygone � dessiner avec les coordonn�s pr�c�dents
		// Transformation du polygone � partir de la translation et de la rotation directement donn�es par le moteur physique
	...
	// M�thode affichant, dessinant un rond en accord avec les donn�es calcul�es par le moteur physique
